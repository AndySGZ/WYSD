...
setj(q)
print("target q   =", np.round(q,4).tolist())
kp = m.actuator_gainprm[act_id,0]
gq = d.qfrc_gravcomp[arm_dof].copy()
print("kp         =", kp)
print("qfrc_gravcomp =", np.round(gq,2).tolist())
print("expected sag (rad) =", np.round(gq/kp, 4).tolist())

# settle without comp
d.ctrl[:6] = q; d.ctrl[6] = 0.3
for _ in range(2000): mujoco.mj_step(m, d)
qa = d.qpos[arm_qpos].copy()
print("after settle q =", np.round(qa,4).tolist())
print("sag (rad)      =", np.round(qa - q, 4).tolist())
print("sag (deg)      =", np.round(np.rad2deg(qa - q), 2).tolist())
print("mid err =", round(np.linalg.norm(mid()-target)*1000,1), "mm")

# settle with gravity comp
setj(q)
d.ctrl[:6] = q + gq/kp
d.ctrl[6] = 0.3
for _ in range(2000): mujoco.mj_step(m, d)
qa2 = d.qpos[arm_qpos].copy()
print("with comp q =", np.round(qa2,4).tolist())
print("with comp sag (deg) =", np.round(np.rad2deg(qa2 - q), 3).tolist())
print("with comp mid err =", round(np.linalg.norm(mid()-target)*1000,1), "mm")
