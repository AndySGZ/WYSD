...
for target_z in [0.44, 0.40, 0.38, 0.366]:
    qg = solve5(np.array([1.4,0.12,target_z]), env.q_home)
    env.set_joints(qg)
    env.set_gripper(open=True)
    for _ in range(500):
        env.data.ctrl[:] = np.concatenate([qg, env.GRIPPER_OPEN])
        mujoco.mj_step(env.model, env.data)
    # count contacts with object while open
    obj_id = env.object_body_id
    open_contacts = sum(1 for c in env.data.contact if env.model.geom_bodyid[c.geom1]==obj_id or env.model.geom_bodyid[c.geom2]==obj_id)
    # close
    for _ in range(800):
        env.data.ctrl[:] = np.concatenate([qg, env.GRIPPER_CLOSE])
        mujoco.mj_step(env.model, env.data)
    obj_id = env.object_body_id
    close_contacts = sum(1 for c in env.data.contact if env.model.geom_bodyid[c.geom1]==obj_id or env.model.geom_bodyid[c.geom2]==obj_id)
    print(f"target_z={target_z}: open_gap={round(env.tooth_gap()*1000,1)} -> close_gap={round(env.tooth_gap()*1000,1)} grip={round(env.get_gripper(),4)} obj={np.round(env.get_object_pos(),3)} open_contacts={open_contacts} close_contacts={close_contacts}")
