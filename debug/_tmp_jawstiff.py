...
def add_stiffness(txt, stiff):
    for j in ["jaw1.1", "jaw1.2", "jaw2.1", "jaw2.2"]:
        # add stiffness and ref to the joint (only the 4 tendon joints)
        txt = re.sub(rf'(<joint name="{j}"[^>]*?)( ?/>)',
                     lambda m: m.group(1) + f' stiffness="{stiff}"' + m.group(2), txt)
    return txt
...
def measure():
    ...
    return np.linalg.norm(mid()-target)*1000, gap*1000, float(d.sensor("gripper_position").data)

try:
    for stiff in [0, 3, 10, 30]:
        txt = add_stiffness(orig, stiff) if stiff else orig
        ARM.write_text(txt, encoding="utf-8")
        try:
            err, gap, sens = measure()
            print(f"jaw stiffness={stiff:3d}: mid_err={err:5.1f}mm gap={gap:5.1f}mm tendon={sens:6.3f}")
        except Exception as e:
            print(f"jaw stiffness={stiff:3d}: FAILED {e}")
finally:
