...
# gravity torque at q
gq = d.qfrc_bias[arm_dof].copy()
print("gravity torque at q =", np.round(gq,3).tolist())

# no gravity comp (baseline ctrl = q)
d.ctrl[:6] = q; d.ctrl[6] = 0.3
for _ in range(1500): mujoco.mj_step(m, d)
print("NO comp:   mid err =", round(np.linalg.norm(mid()-target)*1000,1), "mm, mid=", np.round(mid(),3).tolist())
...
d.ctrl[6] = 0.3
for _ in range(1500): mujoco.mj_step(m, d)
print("grav comp: mid err =", round(np.linalg.norm(mid()-target)*1000,1), "mm, mid=", np.round(mid(),3).tolist())
print("joints =", np.round(d.qpos[arm_qpos],4).tolist())
